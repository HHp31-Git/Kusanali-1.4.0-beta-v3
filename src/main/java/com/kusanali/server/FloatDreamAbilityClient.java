...
public class FloatDreamAbilityClient implements HudRenderCallback {
    ...
    public static void register() {
        ...
        net.fabricmc.fabric.api.client.event.lifecycle.v1.ClientTickEvents.END_CLIENT_TICK.register(client -> {
            if (ModKeySet.ACTIVATE_FLOAT_DREAM.wasPressed()) {
                if (isFloatDreamHeld(client.player)) {
                    ClientPlayNetworking.send(Identifier.of("kusanali", "activate_r_ability"),
            ...
            if (ModKeySet.E_FLOAT_DREAM.wasPressed()) {
                if (isFloatDreamHeld(client.player)) {
                    ClientPlayNetworking.send(Identifier.of("kusanali", "activate_g_ability"),
                            PacketByteBufs.create());
                }
            }
        });
        ...
        // 接收服务器冷却同步
        ClientPlayNetworking.registerGlobalReceiver(Identifier.of("kusanali", "r_cooldown_update"),
                (client, handler, buf, responseSender) -> {
                    long serverCooldownEnd = buf.readLong();
                    client.execute(() -> rCooldownEnd = serverCooldownEnd);
                });

        ClientPlayNetworking.registerGlobalReceiver(Identifier.of("kusanali", "g_cooldown_update"),
                (client, handler, buf, responseSender) -> {
                    long serverCooldownEnd = buf.readLong();
    ...
    private void renderCooldownHud(DrawContext drawContext, TextRenderer textRenderer,
        ...
        if (cooldownEnd == 0 || currentTime >= cooldownEnd) {
            ...
            long totalCooldown = prefix.contains("G") ? 11000 : 30000; // G键11秒，R键30秒
            int percent = (int) (100 - (remaining * 100 / totalCooldown));
