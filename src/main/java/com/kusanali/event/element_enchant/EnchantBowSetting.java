...
public class EnchantBowSetting {
    private static final Set<String> ELEMENTAL_WEAPON_IDS = Set.of(
            MOD_ID + ":anemo_enchant_bow",
            MOD_ID + ":geo_enchant_bow",
            MOD_ID + ":electro_enchant_bow",
            MOD_ID + ":dendro_enchant_bow",
            MOD_ID + ":hydro_enchant_bow",
            MOD_ID + ":pyro_enchant_bow",
            MOD_ID + ":cyro_enchant_bow"
    );
    ...
    private static void enforceSingleElemental(ItemStack stack) {
        ...
        if (nbt == null || !nbt.contains("Enchantments_Bow", 9)) return;

        NbtList old = nbt.getList("Enchantments_Bow", 10);
        ...
        if (changed) {
            nbt.put("Enchantments_Bow", fresh);
            if (fresh.isEmpty()) stack.removeSubNbt("Enchantments_Bow");
        }
    }
}
